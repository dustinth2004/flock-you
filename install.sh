...
apt-get update
# Added libsdl2-dev for pygame audio
apt-get install -y python3-pip python3-venv libglib2.0-dev git gpsd gpsd-clients libsdl2-dev

# WiFi Monitor Mode tools
...
# Check if datasets exist before copying
if [ -d "datasets" ]; then
    cp -r datasets $INSTALL_DIR/
else
    echo "Warning: 'datasets' directory not found. Skipping."
fi
...
echo "To view dashboard: http://<PI_IP>:5000"
